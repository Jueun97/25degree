...
const Mypage = ({ posts, likes, user }) => {
  ...
  return (
    <>
      <Header userId={userId} mypageSign={sign}></Header>
      ...
      <div className={styles.buttons}>
        <button
          className={`${styles.button} ${myPostBtn}`}
          onClick={onClickMypost}
        >
          내 게시물
        </button>
        <button
          className={`${styles.button} ${likesBtn}`}
      ...
      {filteredLikedPosts && (
        <Posts posts={filteredLikedPosts} userId={userId} user={user}></Posts>
      )}
      {filteredPosts && (
        <Posts posts={filteredPosts} userId={userId} user={user}></Posts>
      )}
    </>
  );
};
