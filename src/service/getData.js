...
class GetData {
  ...
  getPost = async () => {
    ...
    const posts = await axios.get(url).then(response => response.data).then(data => data);
    return posts;
  }
  getComment = async () => {
    let url = `${base_url}/Comment`;
    const comments = await axios.get(url).then(response => response.data).then(data => data);
    return comments;
  }
  getLikes = async () => {
